// Summary controller for Buffer And Spring Kit (Lower)

function collectVariants_bask() {
...
export function resetSummary_BufferAndSpringKit() {
    try { document.querySelectorAll('[id^="summaryItems_bufferAndSpringKit"]').forEach(el => { el.style.display = "none"; }); } catch (_) {}
}

export function updateSummary_BufferAndSpringKit() {
    const variants = collectVariants_bask();
    variants.forEach(v => {

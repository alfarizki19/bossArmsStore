// Summary controller for Takedown Pin Set (Lower)

function collectVariants_takedownPinSet() {
    const root = window?.part?.takedownPin;
    const items = [];
...
export function resetSummary_TakedownPinSet() {
    try { document.querySelectorAll('[id^="summaryItems_takedownPinSet"]').forEach(el => { el.style.display = "none"; }); } catch (_) {}
}

export function updateSummary_TakedownPinSet() {

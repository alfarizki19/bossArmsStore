// Summary visibility controller for Upper Receiver

function getQuantityById_UR(variantId) {
...
export function resetSummary_UpperReceiver() {
    const ids = [
        "upperReceiver00100101",
        "upperReceiver00100102"
    ];
    ids.forEach(id => {
...
export function updateSummary_UpperReceiver() {
    const ids = [
        "upperReceiver00100101",
        "upperReceiver00100102"
    ];
    ids.forEach(id => {
        const qty = getQuantityById_UR(id);
        if (qty > 0) {
            const el = document.getElementById(`summaryItems_${id}`);
            if (el) el.style.display = "";

            const meta = getTitleAndPriceById_UR(id);
